...
import { freeze, immerable, isDraft, original } from 'immer';
...
function estimateDays(distanceKm: number, ascent: number, pace = 32) {
...
async function loadFileTrace(file: string) {
    ...
    const format = file.endsWith('.kml') ? 'kml' : file.endsWith('.gpx') ? 'gpx' : 'unknown';

    const gpx = format === 'kml' ? kmlToGpx(content) : content;

    // parse GPX using the gpx package to get exact statistics
    const gpxFile = parseGPX(gpx);
    const statistics = gpxFile.getStatistics();

    // collect flat coordinates and points
    const trackPoints = gpxFile.getTrackPoints();

    const coordinates = trackPoints.map((p) => [p.getLongitude(), p.getLatitude(), p.ele ?? null]);

    const distance = statistics.global.distance.total; // in km

    const maxAltitude = trackPoints.length ? Math.max(...trackPoints.map((p) => p.ele ?? -Infinity)) : -Infinity;

    const ascent = Math.round(statistics.global.elevation.gain);
...
export async function loadTraces() {
    ...
    const traceFiles = files.filter((f) => f.endsWith('.gpx') || f.endsWith('.kml'));
